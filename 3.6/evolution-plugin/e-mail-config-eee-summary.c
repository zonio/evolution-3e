...
#define E_MAIL_CONFIG_EEE_SUMMARY_GET_PRIVATE(obj) \
	...
	((obj), E_TYPE_MAIL_CONFIG_EEE_SUMMARY, EMailConfigEeeSummaryPrivate))

struct _EMailConfigEeeSummaryPrivate {
...
G_DEFINE_DYNAMIC_TYPE (
	EMailConfigEeeSummary,
	e_mail_config_eee_summary,
	E_TYPE_EXTENSION)

static EMailConfigSummaryPage *
mail_config_eee_summary_get_summary_page (EMailConfigEeeSummary *extension)
{
...
static gboolean
mail_config_eee_summary_is_applicable (EMailConfigSummaryPage *page)
{
	ESource *source;
	...
	source = e_mail_config_summary_page_get_account_source (page);
        ...
        eee_server = get_eee_server_hostname (address);
        g_free (eee_server);

        return eee_server != NULL;
}

static void
mail_config_eee_summary_refresh_cb (EMailConfigSummaryPage *page,
                                       EMailConfigEeeSummary *extension)
{
	extension->priv->applicable =
		mail_config_eee_summary_is_applicable (page);

	g_object_notify (G_OBJECT (extension), "applicable");
}

static void
mail_config_eee_summary_commit_changes_cb (EMailConfigSummaryPage *page,
                                              GQueue *source_queue,
                                              EMailConfigEeeSummary *extension)
{
	ESource *source;
	...
	/* If this is not a Eee account, do nothing (obviously). */
	if (!e_mail_config_eee_summary_get_applicable (extension))
		return;
...
static void
mail_config_eee_summary_get_property (GObject *object,
                                         guint property_id,
                                         GValue *value,
                                         GParamSpec *pspec)
{
	switch (property_id) {
		case PROP_APPLICABLE:
			g_value_set_boolean (
				value,
				e_mail_config_eee_summary_get_applicable (
				E_MAIL_CONFIG_EEE_SUMMARY (object)));
			return;
	}
...
static void
mail_config_eee_summary_dispose (GObject *object)
{
	EMailConfigEeeSummaryPrivate *priv;

	priv = E_MAIL_CONFIG_EEE_SUMMARY_GET_PRIVATE (object);
	...
	G_OBJECT_CLASS (e_mail_config_eee_summary_parent_class)->
		dispose (object);
}

static void
mail_config_eee_summary_constructed (GObject *object)
{
	EMailConfigEeeSummary *extension;
	...
	extension = E_MAIL_CONFIG_EEE_SUMMARY (object);

	/* Chain up to parent's constructed() method. */
	G_OBJECT_CLASS (e_mail_config_eee_summary_parent_class)->
		constructed (object);

	page = mail_config_eee_summary_get_summary_page (extension);
	...
	g_signal_connect_after (
		page, "refresh",
		G_CALLBACK (mail_config_eee_summary_refresh_cb),
		extension);

	g_signal_connect_after (
		page, "commit-changes",
		G_CALLBACK (mail_config_eee_summary_commit_changes_cb),
		extension);
	...
	text = _("3_e calendar server has been found for your domain. You can enable\n"
...
static void
e_mail_config_eee_summary_class_init (EMailConfigEeeSummaryClass *class)
{
	GObjectClass *object_class;
	EExtensionClass *extension_class;

	g_type_class_add_private (
		class, sizeof (EMailConfigEeeSummaryPrivate));

	object_class = G_OBJECT_CLASS (class);
	object_class->get_property = mail_config_eee_summary_get_property;
	object_class->dispose = mail_config_eee_summary_dispose;
	object_class->constructed = mail_config_eee_summary_constructed;
...
static void
e_mail_config_eee_summary_class_finalize (EMailConfigEeeSummaryClass *class)
{
}

static void
e_mail_config_eee_summary_init (EMailConfigEeeSummary *extension)
{
	ESource *source;
	ESourceBackend *backend_extension;
	const gchar *extension_name;

	extension->priv = E_MAIL_CONFIG_EEE_SUMMARY_GET_PRIVATE (extension);

	source = e_source_new (NULL, NULL, NULL);
	extension_name = E_SOURCE_EXTENSION_COLLECTION;
	backend_extension = e_source_get_extension (source, extension_name);
	e_source_backend_set_backend_name (backend_extension, "eee");
	extension->priv->collection_source = source;
}

void
e_mail_config_eee_summary_type_register (GTypeModule *type_module)
{
	/* XXX G_DEFINE_DYNAMIC_TYPE declares a static type registration
	 *     function, so we have to wrap it with a public function in
	 *     order to register types from a separate compilation unit. */
	e_mail_config_eee_summary_register_type (type_module);
}

gboolean
e_mail_config_eee_summary_get_applicable (EMailConfigEeeSummary *extension)
{
	g_return_val_if_fail (
		E_IS_MAIL_CONFIG_EEE_SUMMARY (extension), FALSE);
